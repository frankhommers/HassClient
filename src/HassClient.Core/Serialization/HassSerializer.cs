...
namespace HassClient.Core.Serialization
{
  ...
  {
    private static readonly NamingStrategy NamingStrategy = new SnakeCaseNamingStrategy
    ...
    {
      return JsonSerializer.CreateDefault(DefaultSettings);
    }
    ...
    /// <summary>
    ///   Deserializes the <see cref="JRaw" /> object to the specified .NET type using default <see cref="HassSerializer" />
    ///   settings.
    /// </summary>
    ...
    {
      if (value != null) JsonConvert.PopulateObject((string)value.Value, target, DefaultSettings);
    }
    ...
    ///   White-list containing the name of the properties to be included in the <see cref="JObject" />.
    ...
    public static JObject CreateJObject(object value, IEnumerable<string> selectedProperties = null)
    {
      ...
      {
        SelectedPropertiesContractResolver contractResolver =
          CreateContractResolver<SelectedPropertiesContractResolver>();
    ...
    /// <param name="value">
    ///   The snake case <see cref="string" /> representation of the enumeration name or underlying value to
    ///   convert.
    /// </param>
    /// <param name="result">
    ///   When this method returns, result contains an object of type <typeparamref name="TEnum" /> whose value
    ///   is represented by value if the parse operation succeeds. If the parse operation fails, result contains the default
    ///   value
    ///   of the underlying type of <typeparamref name="TEnum" />. Note that this value need not be a member of the
    ///   <typeparamref name="TEnum" /> enumeration. This parameter is passed uninitialized.
    /// </param>
    /// <returns>
    ...
    /// <returns>
    ///   An <see cref="string" /> representing the name of the property once serialized using default
    ///   <see cref="HassSerializer" /> settings.
    /// </returns>
    public static string GetSerializedPropertyName(PropertyInfo propertyInfo)
    {
      string attributePropertyName = propertyInfo.GetCustomAttribute<JsonPropertyAttribute>()?.PropertyName;
