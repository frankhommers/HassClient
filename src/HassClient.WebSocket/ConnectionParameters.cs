...
namespace HassClient.WebSocket
{
  ...
  public class ConnectionParameters
  {
    ...
    /// <summary>
    ///   Gets or sets an <see cref="Uri" /> representing the web socket connection endpoint. (e.g.
    ///   <c>ws://localhost:8123/api/websocket</c>).
    /// </summary>
    public Uri Endpoint
    {
      get => endpoint;
    ...
    public static ConnectionParameters CreateFromInstanceBaseUrl(string instanceBaseUrl, string accessToken)
    {
      if (string.IsNullOrWhiteSpace(instanceBaseUrl))
        throw new ArgumentException($"'{nameof(instanceBaseUrl)}' cannot be null or whitespace",
          nameof(instanceBaseUrl));

      if (string.IsNullOrWhiteSpace(accessToken))
        throw new ArgumentException($"'{nameof(accessToken)}' cannot be null or whitespace", nameof(accessToken));

      UriBuilder uriBuilder = new UriBuilder(instanceBaseUrl);
    ...
    ///   <para>
    ///     More information at
    ///     <see href="https://developers.home-assistant.io/docs/add-ons/communication#home-assistant-core" />.
    ///   </para>
    /// </summary>
    ...
    public static ConnectionParameters CreateForAddonConnection()
    {
      ...
      string supervisorToken = Environment.GetEnvironmentVariable(tokenEnvVar);

      if (string.IsNullOrEmpty(supervisorToken))
        throw new InvalidOperationException(
          $"Error initializing API as Supervisor: Environment variable '{tokenEnvVar}' not found.");
